...
func (app *app) handleProcessing(inputFileName string, uuid string) error {
	...
	// clear the chunks
	err = clearCache(uuid, inputFileName, chunkCount)
	if err != nil {
		return err
	}
...
func clearCache(uuid, inputFileName string, chunkCount int) error {
	for i := 0; i < chunkCount; i++ {
	...
	err := os.Remove(fmt.Sprintf("cache/%s_chunk_list.txt", inputFileName))
	if err != nil {
		return fmt.Errorf("couldn't clear cache: %v", err)
	}
