...
func (awsHandler *AWS) uploadTranscriptFile(uuid string) (string, error) {
	...
	uploadName := fmt.Sprintf("transcripts/%s.csv", uuid)
	fmt.Println(uploadName)

	file, err := os.Open(fmt.Sprintf("cache/%s.csv", uuid))
	if err != nil {
